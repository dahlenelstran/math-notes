...
>1.6) Definition^[Lecture 4, 01-23-2024, [[Introduction to Analysis by Gaughan]]]: Let $f: D \mapsto R$. A point $x_0 \in D$ is a **relative maximum (minimum)** of $f$ if and only if there is a neighborhood $Q$ of $x_0$ such that if $x \in Q \cap D$, then $f(x) \leq f(x_0)$ (for a minimum, $f(x) \geq f(x_0)$). 

>1.7) Theorem^[Lecture 4, 01-23-2024, [[Introduction to Analysis by Gaughan]]]:  Suppose $f: [a,b] \mapsto R$ and suppose $f$ has either a relative max or a relative min at $x_0 \in (a,b)$. If $f$ is differentiable at $x_0$, then $f'(x_0)=0$.
